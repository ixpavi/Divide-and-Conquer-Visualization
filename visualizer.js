...
class Visualizer {
    constructor() {
        ...
        // Layers
        this.layerArray = document.getElementById('array-layer');
        this.layerMatrix = document.getElementById('matrix-layer');
        this.layerCanvas = document.getElementById('canvas-layer');
        this.ctx = this.layerCanvas ? this.layerCanvas.getContext('2d') : null;

        // Recursion Tree
        this.layerTree = document.getElementById('tree-canvas');
        this.ctxTree = this.layerTree ? this.layerTree.getContext('2d') : null;
    ...
    prepareLayer(algoKey, inputMode) {
        ...
        this.reset(false);
        if (this.layerArray) this.layerArray.style.display = 'none';
        if (this.layerMatrix) this.layerMatrix.style.display = 'none';
        if (this.layerCanvas) this.layerCanvas.style.display = 'none';
    ...
    tick() {
        ...
        // Render Frame
        console.log('[DEBUG] Drawing step type:', step.type);
        if (step.type === 'array') this.drawArray(step);
        else if (step.type === 'matrix') this.drawMatrix(step);
        else if (step.type === 'geometry') this.drawGeometry(step);
    ...
    drawGeometry(step) {
        if (!this.layerCanvas || !this.ctx) return;
